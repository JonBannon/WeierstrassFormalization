...
\thanks{Every definition, theorem, and proof in this paper --- including
  both directions of Theorem~\ref{thm:main} and the Gaussian-integer
  realization of Theorem~\ref{prop:Zi} --- has been formally verified in
  the Lean~4 proof assistant, with no \texttt{sorry}s and no axioms
  beyond Mathlib's standard \texttt{propext}, \texttt{Classical.choice},
  and \texttt{Quot.sound}.  An expository blueprint linking each result
  to its Lean formalization is available at
  \url{https://davidvfeldman.github.io/WeierstrassFormalization/blueprint/};
  the Lean source is at
  \url{https://github.com/DavidVFeldman/WeierstrassFormalization}.}
\date{}
...
polynomials with integer coefficients.  One might ask: what if we
consider roots of power series with integer coefficients?  Two immediate
observations change the picture.  First, a non-polynomial integer-coefficient
...
In search of an analogue, one
can ask: what zero sets of holomorphic functions on $\D$ arise as the
zero sets of integer power series?  An \emph{effective divisor} on $\D$
...
nonreal zeros into conjugate pairs.  The main result is that conjugation
invariance is the only arithmetic obstruction.

The main result of this paper is that these two conditions are also
sufficient.
...
representative.  The construction is non-canonical --- different
orderings of the zeros and different rounding choices yield different
realizing functions --- and the coefficient sequences can vary
substantially with these choices.
...
The construction common to both results deforms each classical Weierstrass
elementary factor by a single free parameter.  The governing principle is:

\begin{quote}
\emph{Each modified factor of order $n$ leaves all Taylor coefficients
of degree $\leq n$ unchanged, while shifting the coefficient of
degree $n+1$ by a controlled affine amount.}
\end{quote}

This triangular structure allows coefficients to be rounded to integers
...
\cite{Stout} for background on analytic function algebras.  We have
not found a prior result combining prescribed divisors with
integrality of Taylor coefficients, though we have not exhaustively
searched all adjacent literatures (arithmetic analytic functions,
interpolation in subrings of $H(\D)$, random integer power series).
...
$m = n+1$ is an affine surjection onto $\C$.  The modified factors thus
act as elementary jet-correction operators: each $E_n(z/a;\,c)$ corrects
one new Taylor coefficient without disturbing any previously corrected
one.  The resulting induction resembles Borel-type interpolation or
...
We are unaware of results concerning whether $\mR$ is coherent, integrally
closed, or B\'ezout; we derive only what follows from the main results.
